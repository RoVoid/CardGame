...
wss.on('connection', (ws) => {
    ...
    const player: Player = { uuid, ws, cards: [], usedCards: [], sum: 0, nickname: `Игрок ${players.length}` };
    players.push(player);
    ...
    ws.on('message', (rawData) => {
        try {
            ...
            if (type === 'nickname') {
                ...
                if (!player.cards.includes(cardType)) return;

                const target = players.find((p) => p.uuid === targetUUID);
                if (!target) return;

                const isNumber = ['0', '1', '2', '3', '4'].includes(cardType);
                if ((isNumber && target.uuid !== player.uuid) || (!isNumber && target.cards.length === 0)) return;
            ...
            } else log(`Неизвестное сообщение от ${player.nickname}: (${type})\n${data}`);
        } catch (e) {
            warn('⚠️ Ошибка:', e);
        }
    });
...
function handleSpecial(card: string, from: Player, to: Player) {
    if (card === 'plus') {
        sum++;
        from.sum++;
        let changed = false;
        ...
        if (changed) send(to.ws, 'stats', { cards: to.cards });
        send(from.ws, 'stats', { cards: from.cards, sum: from.sum });
        if (sum > maxSum) endGame();
    } else if (card === 'bin') {
        sum -= to.sum;
        to.sum = 0;
        to.usedCards = [];
...
function startGame() {
    ...
    if (isGame) return log('⚠️ Игра уже начата');
    ...
    for (const pl of players) {
        ...
        pl.usedCards = [];
        pl.sum = 0;
        for (let i = 0; i < 4; i++) pl.cards.push(cards.pop()!);
    }
...
function endGame(termite = false) {
    isGame = false;
    if (!closing) broadcast('gameStats', getGameStats());
    if (termite) {
        log(`🏁 Игра отмена!\n`);
        broadcast('loser');
    } else {
        if (!closing) broadcast('loser', { uuid: players[moveIndex]?.uuid, nickname: players[moveIndex]?.nickname });
...
function broadcast(type: string, data?: object) {
    for (const p of players) send(p.ws, type, data);
}
...
function shuffleCards() {
    ...
    for (const type in cardsTemplate) {
        for (let i = 0; i < cardsTemplate[type]; i++) cards.push(type);
    }
...
function closeServer() {
    ...
    for (const pl of players) {
        try {
            if (pl.ws.readyState === WebSocket.OPEN) pl.ws.close(1000); // 'Сервер завершил работу'
        } catch {}
    }
    wss.close((err) => {
